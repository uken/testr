...
#' @param v0 scale of the sigma_squared parameter. Compare with number of
#'   data samples.
#' @param m0 mean parameter for the normal prior on mu | sigma^2. Ignored if
...
plot_revenue_prior <-
  function(alpha0 = 1,
    ...
    s2 <-
      quantile(rinvgamma2(n, shape = v0 / 2, scale = v0 * s_sq0 / 2),
               c(0.1, 0.5, 0.9, 0.95))
    ...
    if (plot) {
      ...
      curve(
        dinvgamma2(x, shape = v0 / 2, scale = v0 * s_sq0 / 2),
        from = 0,
        to = s2[4],
        n = resolution,
        lwd = 2,
        xlab = "variance",
        ylab = "density",
        main = "Inverse gamma prior over variance"
      )
      ...
      curve(
        dnorm(x, mean = m0, sd = sqrt(s2[1] / k0)),
        xlim = lim,
        n = resolution,
        lwd = 2,
        xlab = "log revenue",
        ylab = "density",
        col = "darkblue",
        main = "Normal prior over mean | variance"
      )
      
      curve(
        dnorm(x, mean = m0, sd = sqrt(s2[2] / k0)),
        xlim = lim,
        add = TRUE,
        n = resolution,
        lwd = 2,
      ...
      curve(
        dnorm(x, mean = m0, sd = sqrt(s2[3] / k0)),
        xlim = lim,
        add = TRUE,
        n = resolution,
        lwd = 2,
      ...
      legend(
        "topright",
        legend = paste("variance = ", round(s2, 1)),
      ...
      curve(
        dbeta(x, shape1 = alpha0, shape2 = beta0),
        from = 0,
        to = 1,
        n = resolution,
        xlab = "conversion_rate rate",
        ylab = "density",
        lwd = 2,
        main = "Beta prior over the conversion_rate rate"
      )
