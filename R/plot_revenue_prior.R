...
plot_revenue_prior <- function(alpha0 = 1,
  ...
  if (!is.null(expected_revenue_converted_users)) {
    m0 <- log(expected_revenue_converted_users)- .5 * v0 * s_sq0 / (v0 + 2)
  }
  ...
  if(plot) {
    ...
    curve(dnorm(x,mean=m0, sd=sqrt(s2[1]/k0)),
          xlim = lim, n = resolution, lwd = 2,
          xlab="log revenue", ylab="density",
          col="darkblue",
          main="Normal prior over mean | variance")

    curve(dnorm(x,mean=m0, sd=sqrt(s2[2]/k0)),
          xlim=lim, add = T, n = resolution,
          lwd = 2, col="darkred")
    ...
    curve(dbeta(x, shape1 = alpha0, shape2 = beta0),
          from = 0, to = 1, n = resolution,
          xlab = "conversion_rate rate", ylab = "density",
          lwd = 2,
