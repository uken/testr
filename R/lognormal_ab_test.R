...
lognormal_ab_test <- function(data,
                              ...
                              s_sq0 = 1,
                              v0 = 5,
                              expected_revenue_converted_users = NULL,
  ...
  if (!is.null(expected_revenue_converted_users)) {
    m0 <-
      log(expected_revenue_converted_users) - 0.5 * v0 * s_sq0 / (v0 + 2)
  }
  ...
  sample.mean <-
    aggregate(revenue ~ ab_group, data = data, FUN = mean)$revenue
  ...
  ngroups <- nrow(data)
  conv.samps <-
    spenders.rev.samps <- rev.samps <- matrix(nrow = ngroups, ncol = nsim)
  ci <-
    matrix(nrow = ngroups,
           ncol = 2,
  ...
  for (g in 1:ngroups) {
    ...
    conv.samps[g, ] <-
      rbeta(nsim,
            nonzero.count[g] + alpha0,
            sample.sizes[g] - nonzero.count[g] + beta0) #samples from the distribution of conversion rates
    spenders.rev.samps[g, ] <-
      draw_log_normal_means(nonzeros, m0, k0, s_sq0, v0, nsim) #samples from the distribution of spenders' revenue
    rev.samps[g, ] <- conv.samps[g, ] * spenders.rev.samps[g, ]
    
    #compute credible intervals
    ci[g, ] <-
      quantile(rev.samps[g, ], probs = c(conf.level / 2, 1 - conf.level / 2))
  }
  
  posterior.mean <- rowMeans(rev.samps)
  prob.winning <-
    rowMeans(rev.samps == matrix(rep(
      apply(rev.samps, FUN = max, MARGIN = 2), ngroups
    ), nrow = ngroups, byrow = TRUE)) # [P(group 1 is best), P(group 2 is best) ...]
  
  #########################
  # visualization
  posterior.samples <-
    data.frame(mean.revenue = as.numeric(t(rev.samps)),
               group = as.factor(rep(groups, each = nsim)))
  
  if (save.hist) {
    hist.data <- data.frame(group = NA,
                            bin = NA,
                            density = NA)
    for (g in groups) {
      h <-
        hist(posterior.samples[posterior.samples[, 'group'] == g, 'mean.revenue'], plot =
               FALSE)
      hist.data <-
        rbind(hist.data,
              data.frame(
  ...
  for (g in 1:ngroups) {
    loss <-
      apply(rev.samps, FUN = max, MARGIN = 2) - rev.samps[g, ] #difference between the MAP mean and the group that is actually the best
    risk[g]  <- mean(loss)
  }
