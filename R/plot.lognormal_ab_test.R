...
#'
#' Plot posterior distributions for a Zero-Inflated Lognormal A/B test using ggplot2
...
plot.lognormal_ab_test <-
  function(x,
           ...
           ...) {
    posterior_samples <-
      data.frame(mean.revenue = as.numeric(t(x$rev.samps)),
                 group = as.factor(rep(x$groups, each = x$nsim)))
    if (is.null(labels)) {
      labels <- x$groups
    }
    gg <-
      ggplot2::ggplot(posterior_samples,
                      ggplot2::aes(x = mean.revenue, fill = factor(group))) +
      ggplot2::geom_density(...) +
      ggplot2::scale_x_continuous(limits = limits) +
      ggplot2::scale_fill_discrete(name = ifelse(length(x$groups) > 1, "Variants", "Variant"),
                                   labels = labels) +
